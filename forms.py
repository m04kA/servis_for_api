...
class RegistrationForm(Form):
    First_name = StringField('Имя', [validators.DataRequired(), validators.Length(min=2, max=25)])
    Second_name = StringField('Фамилия', [validators.DataRequired(), validators.Length(min=2, max=25)])
...
class LoginForm(Form):
    email = StringField('Email-адрес', [validators.DataRequired(), validators.Length(min=6, max=35)])
